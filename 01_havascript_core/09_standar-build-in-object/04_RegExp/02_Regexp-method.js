/*
    ...
    3. m(multiline): ^, $가 패턴에 있는 경우 매 개행마다 각 줄의 시작과 종료를 기준으로 검색 되게 하는 옵션
...
/* 2. test: 정규표현식에서 제공하는 메소드로 매칭 결과를 boolean형태로 반환 */
console.log(/va/.test(target));

/* 3. match: 문자열이 제공하는 메소드로 인수로 정규편식을 주면 패턴을 찾아 배열로 반환
...
const regex1 = /^두/;
console.log(regex1.exec(text)); //null

const regex2 = /^두/m;
console.log(regex2.exec(text));//[ '두', index: 7, input: '첫 번째 줄\n두 번째 줄\n세 번째 줄', groups: undefined ]

const regex3 = /줄$/;
console.log(regex3.exec(text));//[ '줄', index: 19, input: '첫 번째 줄\n두 번째 줄\n세 번째 줄', groups: undefined ]

const regex4 = /줄$/m;
console.log(regex4.exec(text));//[ '줄', index: 5, input: '첫 번째 줄\n두 번째 줄\n세 번째 줄', groups: undefined ]